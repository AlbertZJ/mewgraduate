 ...
 <!--  
   ...
   function checkAll(formName,checkboxName){
	   ...
	   if(e.checked){
	       ...
	       var tds = tr.cells;  
	       var str = "you click ";    
	       /* if(tds[4].innerHTML=="未选"){
	    	   alert("已经被他人选走了，请另选！");
	       }else{ */
	    	    
	       for(var i = 1;i < tds.length;i++){  
	    	   /* switch(i){
	    	   case 1:tds[i].name="title_id";break;
	    	   case 2:tds[i].name="title_name";break;
	    	   case 3:tds[i].name="tech_id";break;
	    	   case 4:tds[i].name="title_state";break;
	    	   case 5:tds[i].name="title_show";break;
	    	   case 6:tds[i].name="stu_id"; 
	    	   }
...
<font face="楷体"   size="6" color="#000">选题界面</font>
	<form name="formName" action = "checkbackselect.jsp" method = "post" onsubmit = "return validate()">
  	<table      border="1"  >
  	  <tr    height="10">
   <th>选择</th> 
		<th>课题编号</th>
  ...
  <input type="radio" name="no"   title="选择/不选择" value="<%=tl.getTitle_id() %>" ></td>  
     ...
     <%}  
    	 /*  String s=tl.getStu_id();
    	   out.println( s );
    	   out.println(stu );
    	   out.println(stu.equals(s) ); */
